...
exports.createFeedback = async (req, res) => {
  try {
    ...
    // Notify admin/manager about new feedback (pending approval)
    try {
      await notificationController.notifyAllAdmins(
        'system',
        'New Feedback Received',
        `New ${populatedFeedback.category} feedback from ${populatedFeedback.guestId.firstName} ${populatedFeedback.guestId.lastName} - Rating: ${populatedFeedback.rating}/5. Status: Pending approval`,
        populatedFeedback._id,
        'Feedback'
      );
    } catch (notifError) {
